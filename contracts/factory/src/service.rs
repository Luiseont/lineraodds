...
mod tests {
    ...
    use super::{FactoryService, FactoryState};

    #[test]
    fn query() {
        let value = 60u64;
        let runtime = Arc::new(ServiceRuntime::<FactoryService>::new());
        let mut state = FactoryState::load(runtime.root_view_storage_context())
            .blocking_wait()
            .expect("Failed to read from mock key value store");
        state.value.set(value);

        let service = FactoryService { state, runtime };
